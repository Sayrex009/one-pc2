    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(
          "https://pc.onepc.uz/api/v1/product/discounted-product/list/"
        );
      ...
      <div className="w-full px-5 mx-auto max-w-[1320px]">
        <div className="flex justify-between items-center mb-5 mt-20 md:mb-[30px]">
          <h1 className="text-[#111] text-[18px] md:text-[30px] font-semibold border-b-[3px] border-b-[#E2231F]">
            Chegirmdagi mahsulotlar
          </h1>
        </div>
        {error && (
          <p className="text-center text-[#FF0000]">Hatolik!: {error}</p>
        )}
        ...
        <div className="relative px-2">
          <Slider {...settings} className="discounted-products-slider">
            {products.map((product) => {
              let imageUrl =
                product.image_uz || product.image || product.image_url || "";
